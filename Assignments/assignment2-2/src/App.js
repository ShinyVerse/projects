...
class App extends Component {
  ...
  render() {
    let charList = this.state.userInput.split('').map((letter, index) => {
      return (
        <Char character={letter} key={index} click={(index) => {this.deleteCharacterHandler(index)}}/>
      )});

    return (
      <div className="App">
        <input type="text" onChange={(event) => this.userInputChange(event)} value={this.state.userInput}/>
        <p>{this.state.userInput}</p>
        <Validation userInput={this.state.userInput} />
        {charList}
        <ol>
            <li>Create an input field (in App component)
               with a change listener which outputs the length of the entered text below it (e.g. in a paragraph).</li>
